...
bins_directory = sys.argv[2]
reads_on_contigs_alignment = sys.argv[3]
R1_path = sys.argv[4]
R2_path = sys.argv[5]
output_plot = sys.argv[6]
output_text = sys.argv[7]
...
bin_to_quality = pd.read_csv(
    checkm_report,
    sep='\t',
    skiprows=1,
    names=[
        "Bin ID", "Completeness", "Contamination", "Completeness_Model_Used",
        "Translation_Table_Used", "Coding_Density", "Contig_N50",
        "Average_Gene_Length", "Genome_Size", "GC_Content",
        "Total_Coding_Sequences", "Contigs", "Max_Contig_Length",
        "Additional_Notes"
    ]
)

bin_to_quality["Quality"] = bin_to_quality.apply(
    lambda x: GSCS_quality(x['Contamination'], x['Completeness'], x['Contigs']),
    axis=1
)

bin_to_quality = {
    bin_id: quality
    for bin_id, quality in zip(bin_to_quality["Bin ID"], bin_to_quality["Quality"])
}
...
# III) : get the quality of the bin containing the contigs that each read is aligned to,
# with the length of the alignments
reads_to_quality = {}
...
for read_name, alignments in reads_to_quality.items():
    ...
    # Remove overlaps between alignments of different qualities,
    # and store the alignment lengths
    for alignment in merged_alignments:
        ...
        # If not empty, add the new alignment
...
### Plotting ###
output_text_dir = os.path.dirname(output_text)
if output_text_dir:
...
for q in qualities:
    ...
    with open(output_text, 'a') as output_file:
        ...
        output_file.write(
            "aligned reads over total reads count ratio : {:.2f}% ({}/{})\n".format(
                aligned_reads_count_ratio[q] * 100,
                aligned_reads_count[q],
                reads_count
            )
        )
        output_file.write(
            "alignement length over total reads length ratio : {:.2f}% ({}/{})\n".format(
                alignements_length_ratio[q] * 100,
                alignment_lengths[q],
                reads_length
            )
        )
...
df = pd.concat([
    pd.DataFrame(aligned_reads_count_ratio, index=["aligned_reads_count_ratio"]),
    pd.DataFrame(alignements_length_ratio, index=["alignement_length_ratio"])
])
